...
export async function listSignups() {
  ...
  return Object.values(db.data.accounts ?? {})
    .map(({ email, name, createdAt, creatorCode, lastActiveAt }) => ({
      email,
      name: name ?? null,
      createdAt: createdAt ?? null,
      creatorCode: creatorCode ?? null,
      // Last request this account made, stamped (throttled) in
      // lib/user-data.js. Null until they use the app after this shipped.
      lastActiveAt: lastActiveAt ?? null,
      // What this account has spent of its shared-key allowance
      // (lib/claude-spend.js), so the owner can see where the money went.
      // Lives in the user bucket, not the account record, so it is joined
      // here rather than destructured above.
      sharedSpendUsd:
        (db.data.users?.[email]?.sharedClaude?.micros ?? 0) / 1_000_000,
    }))
