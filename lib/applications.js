...
import { formatLearnings, getRelevantLearnings, tagsFromProfile } from "@/lib/learnings";
...
export async function draftApplicationFor(data, job) {
  ...
  const coverLetter = await askClaudeText({
    ...
    prompt: `Candidate profile:\n${JSON.stringify(data.profile, null, 2)}\n\nJob:\n${job.title} at ${job.company}\n${job.description}${formatInsights(data.insights)}${formatLearnings(learnings)}\n\nWrite a tailored cover letter for this application.`,
  });
  ...
  const application = {
    ...
    // Which global patterns were injected into this draft — the status
    // lifecycle (interviewing/hired/rejected) grades them later.
