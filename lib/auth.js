...
// configured we generate one and persist it so sessions survive restarts —
// zero-config, but overridable by setting AUTH_SECRET. Where it persists
// depends on the host: locally it goes to .env.local; on serverless hosts
// the filesystem is read-only AND every instance must agree on the secret
// (or sessions die whenever a request lands elsewhere), so the first boot
// stores one in Redis (SET NX) and everyone reads it back.
let secret = process.env.AUTH_SECRET;
if (!secret && redisConfig()) {
  try {
    const SECRET_KEY = "jobpilot:auth-secret";
    await redisCommand(["SET", SECRET_KEY, randomBytes(32).toString("hex"), "NX"]);
    secret = await redisCommand(["GET", SECRET_KEY]);
    process.env.AUTH_SECRET = secret;
  } catch (err) {
    console.warn(`[auth] Couldn't read/store AUTH_SECRET in Redis (${err.message}).`);
  }
}
if (!secret) {
  ...
  if (process.env.VERCEL) {
    console.warn(
      "[auth] No AUTH_SECRET and no Redis: generated a per-instance secret. " +
        "Sessions will break across serverless instances — set an AUTH_SECRET environment variable in Vercel."
    );
  } else {
