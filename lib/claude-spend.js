...
/**
 * Is the caller the owner? They are never metered.
 *
 * The "shared" key is the owner's own key — it is their ANTHROPIC_API_KEY in
 * their .env.local, billed to their card. Spending a $1 allowance against
 * their own credits and then dropping them to the free model is the meter
 * misreading whose money it is protecting.
 */
export async function callerIsOwner() {
  return isOwnerAccount(null, await currentEmail());
}
...
export async function readAllowance() {
  ...
  // The owner has no allowance to run down, so the meter must never report
  // one — `exhausted: true` here would drop them to the free model.
  if (isOwnerAccount(null, email)) {
    return {
      spentUsd: 0,
      remainingUsd: Infinity,
      allowanceUsd: Infinity,
      calls: db.data.users[email]?.sharedClaude?.calls ?? 0,
      exhausted: false,
      exhaustedAt: null,
      uncapped: true,
    };
