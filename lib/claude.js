...
// matching calls. Haiku 4.5 does NOT support adaptive thinking or the
// `effort` parameter (both 400), but it DOES support classic extended
// thinking (budget_tokens) and the JSON-schema structured outputs below.
// Switch to `claude-sonnet-5` or `claude-opus-4-8` (with adaptive thinking)
// if verdict quality degrades.
export const MODEL = "claude-haiku-4-5";

// Extended thinking on every call: the model reasons privately before
// answering, which sharpens match verdicts and reviews. budget_tokens is a
// CEILING, not a floor (min 1024, must stay under max_tokens) — but thinking
// bills as output tokens ($5/1M), so keep it modest; 4000 ≈ 2¢ worst-case
// per call. Remove `thinking` from the calls below to go back to no-thinking.
