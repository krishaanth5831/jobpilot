...
export const emptyUserData = () => ({
  ...
  sharedClaude: null, // spend against the server's Claude key (lib/claude-spend.js): { micros, calls, firstUsedAt, exhaustedAt }

  // --- Subscription (lib/tiers.js, lib/entitlements.js) ---
  // No `tier` field: the tier is DERIVED by effectiveTier() from the three
  // fields below, so a stale copy can never disagree with Stripe.
  subscription: null, // { tier, status, priceId, interval, currentPeriodEnd, cancelAtPeriodEnd }
  grandfathered: false, // signed up before the paywall flipped — keeps Pro free, permanently
  billingAnchorAt: null, // ISO — monthly counters reset on this day-of-month, not the 1st
  stripeCustomerId: null, // set on first checkout; survives cancellation so the portal keeps working
  usage: {}, // per-feature counters: { [feature]: { key: "2026-07-30" | "life", n } }

  // --- First run (lib/onboarding.js) ---
  // `onboarding` is the signup questionnaire: { version, completedAt, answers }.
  // Null means it has not been answered, which is what puts a new account into
  // the flow. Distinct from `onboardingDismissed`, which is the separate
  // first-run checklist on the home page.
  onboarding: null,
  tourCompletedAt: null, // ISO — the dock walkthrough, shown once, after the questionnaire
  upgradePromptAt: null, // ISO — last time the upgrade prompt was shown or dismissed
  ...
  resumeReview: null, // the ATS health result (lib/resume-health/): the full HealthResult plus the derived fields the resume studio renders
  ...
  interviewPrep: {}, // per-job interview Q&A keyed by jobId: { questions, createdAt }
