...
/** Agentic browser auto-apply — its own flag, and not built yet. */
export function agenticAutoApplyEnabled() {
  const raw = (process.env.AGENTIC_AUTO_APPLY_ENABLED ?? "").trim().toLowerCase();
  return raw === "true" || raw === "1";
}
 ...
 *
 * A live Stripe subscription wins. Otherwise a grandfathered account — one
 * that signed up before the paywall flipped — keeps Pro permanently and free.
 * Everyone else is Free.
 */
export function effectiveTier(bucket) {
 ...
 */
export function monthPeriodStart(anchorISO, now = new Date()) {
...
export function periodKey(period, anchorISO, now = new Date()) {
 ...
 */
export function resumeVersionCount(bucket) {
 ...
 */
export async function canUseFeature(userId, feature, { now = new Date() } = {}) {
  assertNeverGated(feature);

  const db = await getDb();
  const bucket = db.data.users?.[userId] ?? null;
  const tier = effectiveTier(bucket);
 ...
 */
export function upgradeResponse(verdict) {
...
export async function usageSummary(userId, opts = {}) {
  ...
  for (const feature of Object.values(FEATURES)) {
    const v = await canUseFeature(userId, feature, opts);
    out[feature] = {
      used: v.used,
      limit: v.limit,
  ...
  const db = await getDb();
  const bucket = db.data.users?.[userId] ?? null;
  return { tier: effectiveTier(bucket), grandfathered: Boolean(bucket?.grandfathered), features: out };
}
