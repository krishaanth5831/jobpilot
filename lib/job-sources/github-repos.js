...
// descriptions, so entries are screened from listing metadata. Location
...
// The apply link position varies by repo: jobright puts it in the title cell,
// speedyapply/zapply/vanshb03 in a later "apply" column. Prefer the title
// cell's link, then the last non-image link in the row.
...
function parseSimplify(jsonText, repo) {
  return JSON.parse(jsonText)
    ...
    .map((l) => ({
      ...
      employmentHint: repo.kind === "internship" ? "internship" : "graduate",
      queriedCountry: null,
    }));
}

export async function searchGithubRepos({ role, limit = 12 }) {
