...
// Location filtering is done by the caller (index.js).

import { cleanDescription } from "./clean.js";
import { roleTerms, titleRelevance } from "./relevance.js";

// Company slug → display name. A board that 404s is skipped.
const COMPANIES = {
  palantir: "Palantir",
  spotify: "Spotify",
  mistral: "Mistral AI",
...
export async function searchLever({ role, limit = 12 }) {
  ...
  for (const r of boards) {
    ...
    for (const posting of r.value.postings) {
      const { score, relevant } = titleRelevance(posting.text ?? "", terms);
      if (relevant) scored.push({ ...r.value, posting, score });
    }
  }
  ...
  return scored.slice(0, limit).map(({ slug, name, posting }) => {
    ...
    return {
      ...
      employmentHint: posting.categories?.commitment ?? "",
      queriedCountry: null,
    };
  });
}
