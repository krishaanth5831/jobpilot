...
// When no country is given, we brute-force the major job markets and let the
// user filter afterward. Two lists because JSearch's free quota is tight
// (keep it to the biggest markets) while Adzuna is generous (cast wider).
export const BRUTE_JSEARCH = ["us", "gb", "de", "fr", "nl", "in", "ca", "au"];
export const BRUTE_ADZUNA = ["us", "gb", "ca", "de", "fr", "nl", "es", "it", "in", "au", "sg", "br", "ch", "za"];

// Every market a worldwide search touches, as display names — the UI offers
// all of them in the country filter so you can drill into any (even one that
// came back empty) and trigger a thorough search for it.
export const MAJOR_MARKET_NAMES = [...new Set([...BRUTE_JSEARCH, ...BRUTE_ADZUNA])]
